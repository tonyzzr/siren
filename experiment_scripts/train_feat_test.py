...
def get_feat_tensor(sidelength = 244,
                    feat_path = './experiment_scripts/hr_feat.pt'):
    feat = torch.load(feat_path) # (b, c, h, w)
    return feat[0, ...].cpu() 

class FeatureFitting(Dataset):
    def __init__(self, sidelength=244, 
                 feat_path = './experiment_scripts/hr_feat.pt')-> None:
        super().__init__()
        feat = get_feat_tensor(sidelength, feat_path)

        c, h, w = feat.shape
        self.pixels = feat.permute(1, 2, 0).view(-1, c)
...
if __name__ == "__main__":
    ...
    for step in range(total_steps):
        ...
        # downsample the model output to match lr
        model_output_reshaped = model_output.contiguous().view(1, 224, 224, hr_feat_dim) # reshape to (b, c, h, w)

        # print(model_output_reshaped.shape)
        model_output_reshaped = model_output_reshaped.permute(0, 3, 1, 2)

        # input()

        pool = nn.AvgPool2d(kernel_size=16)
        lr_model_output = pool(model_output_reshaped)

        # print(lr_model_output.shape)
        lr_model_output = lr_model_output.permute(0, 2, 3, 1)
        # print(lr_model_output.shape)
        lr_model_output = lr_model_output.view(1, 196, hr_feat_dim)
        # print(lr_model_output.shape)
